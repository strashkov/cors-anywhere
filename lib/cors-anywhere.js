 ...
 */
function proxyRequest(req, res, proxy, options) {
      ...
      pipe: function(proxyReq) {
        ...
        proxyReq.on = function(eventName, listener) {
          ...
          return proxyReqOn.call(this, 'response', function(proxyRes) {
            if (onProxyResponse(proxy, proxyReq, proxyRes, req, res, options)) {
              try {
                listener(proxyRes);
              } catch (err) {
 ...
 */
function onProxyResponse(proxy, proxyReq, proxyRes, req, res, options) {
  ...
  const cacheKey = `${proxyReq.host}${proxyReq.path}`;

  if (options.cacheJson && statusCode !== 200 && cache.has(cacheKey)) {
    req.removeAllListeners();
    proxyReq.removeAllListeners('error');
    proxyReq.once('error', function catchAndIgnoreError() {});
    proxyReq.abort();
  ...
  if (
    options.cacheJson &&
    statusCode === 200 &&
    ['application/json', 'text/json'].includes(proxyRes.headers['content-type'])
  ) {
    let body;
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    proxyRequest(req, res, proxy, options);
  };
}
